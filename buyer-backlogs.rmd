...
# Filtering out buyer KL. See comment around L112.
backlog_plot_inv <- backlog_plot %>% 
  filter(Category == "INV" & Buyer != "KL") %>% 
  plot_ly() %>% 
  add_bars(y = ~Buyer,
          x = ~Count,
          color = ~Type,
...
# KL Info, requested to be removed from the graphs due to constant out-lying.
kl_count_table = backlog_plot %>% filter(Buyer == "KL")
kl_actionable_count = (kl_count_table %>% filter(Type == "Actionable") %>% select(Count))[[1]]
kl_out_to_bid_count = (kl_count_table %>% filter(Type == "Out-To-Bid") %>% select(Count))[[1]]
kl_on_hold_count = (kl_count_table %>% filter(Type == "On Hold") %>% select(Count))[[1]]
```
*Buyer KL Info: Actionable `r kl_actionable_count`, On Hold `r kl_on_hold_count`, Out-To-Bid `r kl_out_to_bid_count`.*
